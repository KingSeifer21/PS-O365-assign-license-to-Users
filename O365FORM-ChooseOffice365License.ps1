...
#Scripts asks for the UserPrincipleName = Amanuyl Ok = aok@deanone.nl
...
do {
    ...
    switch -Regex ( $choice ) {
        "A"
        {
            ...
            Set-MsolUserLicense -UserPrincipalName $UserInOffice365 -addLicense "onexs:O365_BUSINESS_PREMIUM"
        }
        
        "B"
        {
            write-host "You entered 'B'"
            Set-MsolUserLicense -UserPrincipalName $UserInOffice365 -addLicense "onexs:ENTERPRISEPACK"
        }

        "C"
        {
            write-host "You entered 'C'"
            Set-MsolUserLicense -UserPrincipalName $UserInOffice365 -addLicense "onexs:VISIOCLIENT"
        }

        "D"
        {
            write-host "You entered 'D'"
            Set-MsolUserLicense -UserPrincipalName $UserInOffice365 -addLicense "onexs:EXCHANGESTANDARD"
        }
    }
